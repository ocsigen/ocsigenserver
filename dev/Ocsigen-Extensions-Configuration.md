...
raises [`Error_in_config_file`](./Ocsigen-Extensions.md#exception-Error_in_config_file) If an element (resp. attribute) occurs which is not specified by the element (resp. attribute) parameter and no function other\_elements (resp. other\_attributes) is provided
```ocaml
