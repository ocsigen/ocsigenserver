...
val make : 
  ...
  address:Ocsigen.Config.Socket_type.t ->
  port:int ->
  ssl:bool ->
...
```
```ocaml
val address : t -> Ocsigen.Config.Socket_type.t
```
```ocaml
...
```
The bare client IP address (no scheme prefix), suitable for the `%h` field of a Common/Combined access-log line. Returns `"-"` when the address is unknown.

```ocaml
val set_remote_user : t -> string -> unit
```
`set_remote_user r user` records the name of the user authenticated for this request (typically by `Authbasic`). It is stored in the request cache and reported as the `%u` field of the access log.

```ocaml
val remote_user : t -> string option
```
The user authenticated for this request, if any. See [`set_remote_user`](./#val-set_remote_user).
