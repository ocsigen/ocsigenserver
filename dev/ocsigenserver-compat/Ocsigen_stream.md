...
```ocaml
type 'a stream = 'a Ocsigen_base.Ocsigen_stream.stream
```
```ocaml
type 'a step = private 'a Ocsigen_base.Ocsigen_stream.step = 
...
```ocaml
type 'a t = 'a Ocsigen_base.Ocsigen_stream.t
```
```ocaml
type outcome = [ 
  | `Success
...
```ocaml
module StringStream = Ocsigen_base.Ocsigen_stream.StringStream
```