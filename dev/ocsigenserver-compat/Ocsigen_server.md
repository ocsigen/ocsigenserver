...
val start : 
  ...
  ?ssl_info:Ocsigen.Config.ssl_info option ->
  ...
  ?maxretries:int ->
  Ocsigen.Extensions.host_config list ->
  unit
```
...
Options behave exactly like their `configuration file` counterparts.
...
val host : 
  ...
  ?do_not_serve_404:Ocsigen.Extensions.do_not_serve ->
  ?do_not_serve_403:Ocsigen.Extensions.do_not_serve ->
  ?uploaddir:string option ->
  ?maxuploadfilesize:int64 option ->
  instruction list ->
  Ocsigen.Extensions.host_config
```
