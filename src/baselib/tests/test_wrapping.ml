...
type a = {a : float; a_wrap : a Ocsigen_wrap.wrapper}
...
type b = {b : string; ba : a; b' : string; b_wrap : b Ocsigen_wrap.wrapper}

let b_wrap () = Ocsigen_wrap.create_wrapper (fun t -> t.b, t.ba)
...
let b'_wrap () = Ocsigen_wrap.create_wrapper (fun t -> t.b, a 1.2)
...
let ( -- ) x y =
  let rec aux y x acc = if x > y then acc else aux (y - 1) x (y :: acc) in
  aux y x []
...
type d = {da : int; dw : int Weak.t; dm : d Ocsigen_wrap.wrapper}
...
type toto = {a : float; mtoto : toto Ocsigen_wrap.wrapper}
type t = {v1 : int; v2 : toto; v3 : int React.event; mt : t Ocsigen_wrap.wrapper}
...
type t1 = {t1a : float; t1mark : t1 Ocsigen_wrap.wrapper}

type t2 =
  {t2t1 : t1; t2f : (int ref -> unit) option; t2mark : t2 Ocsigen_wrap.wrapper}
