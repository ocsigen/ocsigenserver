...
let badconfig fmt =
  Printf.ksprintf (fun s -> raise (Error_in_config_file s)) fmt
...
let join_do_not_serve d1 d2 =
  ...
  ; do_not_serve_extensions =
      d1.do_not_serve_extensions @ d2.do_not_serve_extensions }
...
let do_not_serve_to_regexp d =
  ...
  with Not_found -> (
    ...
    and bind f = function None -> None | Some v -> Some (f v) in
    ...
    and regexps = bind paren regexps in
    let files = bind (Printf.sprintf ".*/(%s)") files
    and extensions = bind (Printf.sprintf ".*\\.(%s)") extensions in
    let l =
      List.fold_left
        (fun r -> function None -> r | Some v -> v :: r)
        []
        [files; extensions; regexps]
    in
    let regexp =
      if l = []
      then (* This regexp should not never match *) "$^"
...
let default_config_info () =
  ...
  ; default_httpport = Config.get_default_port ()
  ; default_httpsport = Config.get_default_sslport ()
  ; default_protocol_is_https = false
  ; mime_assoc = Ocsigen_http.Charset_mime.default_mime_assoc ()
  ; charset_assoc =
      Ocsigen_http.Charset_mime.empty_charset_assoc
        ?default:(Config.get_default_charset ())
        ()
...
let get_hostname {request_info; request_config = {default_hostname; _}; _} =
  ...
  then default_hostname
  else
    match Request.host request_info with
...
let new_url_of_directory_request request ri =
  ...
  let ssl = Request.ssl ri in
  let scheme = if ssl then "https" else "http"
  and host = get_hostname request
  and port =
    let port = get_port request in
    if port = if ssl then 443 else 80 then None else Some port
  and path =
    let path = Request.path_string ri in
    if path.[String.length path - 1] = '/' then path else path ^ "/"
  and query = Request.get_params ri in
...
let site_match request (site_path : string list) url =
  ...
  let rec aux site_path url =
    ...
    | _ -> None
  in
  match site_path, url with [], [] -> Some [] | _ -> aux site_path url
  ...
  let rec aux cookies_to_set = function
    ...
    | Ext_found r ->
        ...
        g Ocsigen_cookie_map.empty
          (Req_found (ri, Response.add_cookies r' cookies_to_set))
    | Ext_found_continue_with r ->
        r () >>= fun (r', req) ->
        g Ocsigen_cookie_map.empty
          (Req_found (req, Response.add_cookies r' cookies_to_set))
...
let rec compose = function
  ...
  | e :: rest -> compose_step e (compose rest)

let fun_end = ref (fun () -> ())
let fun_exn = ref (fun exn : string -> raise exn)
    ...
    | xmltag :: ll -> (
      ...
      | Bad_config_tag_for_extension t ->
          (* This case happens only if no extension has recognized the
           tag at all *)
          badconfig "Unexpected tag <%s> inside <site dir=\"%s\">" t
            (Url.string_of_url_path ~encode:true path)
      | Error_in_config_file _ as e -> raise e
      | e ->
          badconfig "Error while parsing configuration file: %s"
            (try !fun_exn e with e -> Printexc.to_string e))
  in
  let r =
    try parse_config l with e -> !fun_end (); raise e
...
let site_ext ext_of_children charset path cookies_to_set = function
  ...
  | Req_not_found (e, oldri) -> (
      ...
      in
      match site_match oldri path (Request.path oldri.request_info) with
      | None ->
          Logs.info ~src:section (fun fmt ->
            fmt "site \"%s\" does not match url \"%s\"."
              (Url.string_of_url_path ~encode:true path)
              (Url.string_of_url_path ~encode:true
                 (Request.path oldri.request_info)));
      ...
      | Some sub_path -> (
          ...
          let ri =
            { oldri with
              request_info =
                Request.update oldri.request_info
...
and parse_config_aux =
  Url.path -> parse_host -> parse_fun -> Xml.xml -> extension

let _extension_void_fun_site : parse_config =
...
let register, parse_config_item, get_init_exn_handler =
  ...
  ( (fun ?fun_site
      ...
      () ->
      if respect_pipeline then Config.set_respect_pipeline ();
      (match fun_site with
      | None -> ()
      | Some fun_site -> (
            ...
            fun path host conf_info ->
              ...
              fun path parse_host ->
                ...
                fun parse_config config_tag ->
                  try oldf parse_config config_tag
                  with Bad_config_tag_for_extension _c ->
                    newf parse_config config_tag));
      (match end_init with
      | Some end_init -> fun_end := Ocsigen_base.Lib.comp end_init !fun_end
      | None -> ());
...
let register
      ...
      ()
  =
  Ocsigen_base.Loader.set_module_init_function name (fun () ->
    (match init_fun with
...
module Configuration = struct
  type attribute' =
    {attribute_obligatory : bool; attribute_value_func : string -> unit}
  ...
  type element' =
    ...
    ; other_elements :
        (string -> (string * string) list -> Xml.xml list -> unit) option
  ...
  let check_element_occurrence ~in_tag elements = function
    | name, {obligatory = true; _} ->
        let corresponding_element = function
          ...
          | _ -> false
        in
        if not (List.exists corresponding_element elements)
        then
          raise
            (Error_in_user_config_file
  ...
  let process_attribute
    ...
    with Not_found -> (
      ...
      | Some spec_other_attributes -> spec_other_attributes attribute value
      | None ->
          raise
            (Error_in_user_config_file
    ...
    | Xml.Element (name, attributes, elements) -> (
      try
        ...
        List.iter
          ...
          spec.attributes;
        List.iter (check_element_occurrence ~in_tag:name elements) spec.elements;
        spec.init ();
        List.iter
          (process_attribute ~in_tag:name ~attributes:spec.attributes
             ?other_attributes:spec.other_attributes)
          attributes;
        List.iter
          (process_element ~in_tag:name ~elements:spec.elements
             ?pcdata:spec.pcdata ?other_elements:spec.other_elements)
          elements
      with Not_found -> (
...
let ( start_initialisation
    , during_initialisation
    , end_initialisation
    , get_numberofreloads )
  =
...
let host_match ~(virtual_hosts : virtual_hosts) ~host ~port =
  let port_match = function None -> true | Some p -> p = port in
  match host with
  ...
  | Some host ->
      let host_match regexp =
        Ocsigen_base.Lib.Netstring_pcre.string_match regexp host 0 <> None
      in
...
let compute_result ?(previous_cookies = Ocsigen_cookie_map.empty) request_info =
    ...
    | (virtual_hosts, request_config, host_function) :: l
      when host_match ~virtual_hosts ~host ~port -> (
        ...
        | Ext_found r | Ext_found_stop r ->
            r () >>= fun r' ->
            Lwt.return (Response.add_cookies r' cookies_to_set)
  ...
  and fold_hosts_limited sites cookies_to_set request_info =
    Request.incr_tries request_info;
    if Request.tries request_info > Config.get_maxretries ()
...
let user_dir_regexp =
  Ocsigen_base.Lib.Netstring_pcre.regexp "(.*)\\$u\\(([^\\)]*)\\)(.*)"

let parse_user_dir s =
  match Pcre.full_split ~rex:user_dir_regexp ~max:(-1) s with
  | [Pcre.Delim _; Pcre.Group (1, s1); Pcre.Group (2, u); Pcre.Group (3, s2)] ->
      Withdir (s1, u, s2)
  | _ -> Nodir s
...
let ( >|! ) v f = match v with None -> raise Not_concerned | Some v -> f v

let find_redirection regexp full_url dest r =
  if full_url
  then
    Request.host r >|! fun host ->
    let path =
      let full_path = Request.path_string r in
      match Request.query r with
      | None -> full_path
      | Some g -> full_path ^ "?" ^ g
    in
    let path =
    ...
    Ocsigen_base.Lib.Netstring_pcre.global_replace regexp dest path
  else
    let path =
      let sub_path = Request.sub_path_string r in
      match Request.query r with
      | None -> sub_path
      | Some g -> sub_path ^ "?" ^ g
    in
