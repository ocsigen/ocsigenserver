...
let update
  ...
  and r_cookies_override =
    ...
    | Some _ -> cookies_override
    | None -> r_cookies_override
  and r_meth = match meth with Some meth -> meth | None -> r_meth
  and r_original_full_path, r_uri =
    match uri with
    | Some uri ->
        ( (match full_rewrite, r_original_full_path with
          | true, _ -> None
          | false, Some _ -> r_original_full_path
          | false, _ -> Some (Uri.path (Lazy.force u_uri)))
        , make_uri uri )
    | None -> r_original_full_path, r_uri
  in
  let r_uri =
    ...
    | Some l ->
        ...
        let u_uri =
          lazy
            (Uri.with_query (Lazy.force r_uri.u_uri) (Lazy.force u_get_params))
        in
        {r_uri with u_uri; u_get_params; u_get_params_flat = lazy l}
    | None -> r_uri
  in
  { r with
    r_ssl
...
let uri {r_uri = {u_uri; _}; _} = Lazy.force u_uri

let to_cohttp ({r_meth; r_encoding; r_version; r_headers; _} as r) =
  Cohttp.Request.make ~meth:r_meth ~encoding:r_encoding ~version:r_version
    ~headers:r_headers (uri r)
...
let query {r_uri = {u_uri; _}; _} = Uri.verbatim_query (Lazy.force u_uri)
let get_params {r_uri = {u_get_params; _}; _} = Lazy.force u_get_params
...
let force_post_data ({r_body; _} as r) s i =
  match !r_body with
  | `Parsed post_data -> Some post_data
  | `Unparsed body -> (
    match content_type r with
    | Some content_type -> (
      match post_data_of_body ~content_type body with
      | Some f ->
          let v = f s i in
          r.r_body := `Parsed v;
          Some v
