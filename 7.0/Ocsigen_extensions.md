...
type answer = 
  ...
  | Ext_found_continue_with of unit -> (Ocsigen_response.t * request) Lwt.t (* Same as Ext_found but may modify the request. *)
  | Ext_found_continue_with' of Ocsigen_response.t * request (* Same as Ext_found_continue_with but does not allow to delay the computation of the page. You should probably not use it, but for output filters. *)
```
```ocaml
and request_state = 
